...
export class EditQuestionnaireComponent implements OnInit {
  ...
  patchSelectedQuestion() {
    ...
    this.editQuestionnaireForm.controls['questionID'].setValue(
      this.data.selectedQuestion.questionID,
    );
    this.editQuestionnaireForm.controls['questionTypeID'].setValue(
      this.data.selectedQuestion.questionTypeID,
    );
    this.editQuestionnaireForm.controls['questionType'].setValue(
      this.data.selectedQuestion.questionType,
    );
    this.editQuestionnaireForm.controls['questionName'].setValue(
      this.data.selectedQuestion.question,
    );
    this.editQuestionnaireForm.controls['questionRank'].setValue(
      this.data.selectedQuestion.questionRank,
    );
    this.editQuestionnaireForm.controls['answerType'].setValue(
      this.data.selectedQuestion.answerType,
    );
    ...
    if (this.data.selectedQuestion.answerType === 'Free Text') {
      ...
      this.editQuestionnaireForm.controls['questionWeight'].setValue(
        this.data.selectedQuestion.questionWeightage,
      );
      let j = 0;
      for (let i = 0; i < this.data.selectedQuestion.qvalues.length; i++) {
        if (this.data.selectedQuestion.qvalues[i].deleted === false) {
          this.addOptField(j);
          this.answerOptions.at(j).patchValue({
            questionValuesID:
              this.data.selectedQuestion.qvalues[i].questionValuesID,
            option: this.data.selectedQuestion.qvalues[i].option,
            optionWeightage:
              this.data.selectedQuestion.qvalues[i].optionWeightage,
            deleted: this.data.selectedQuestion.qvalues[i].deleted,
          });
          j++;
        }
  ...
  weightageInput() {
    const value = this.editQuestionnaireForm.controls['questionWeight'].value;
    // if (value == undefined) {
    // }
  ...
  optionweightage(index: any) {
    const questionvalue = this.editQuestionnaireForm.value.answerOptions;
  ...
  deleteOptionField(idx: any) {
    ...
    console.log(
      'questionWeight2',
      this.answerOptions.at(idx).value.optionWeightage,
    );
    ...
    if (
      this.answerOptions.at(idx).value.questionValuesID === null ||
      this.answerOptions.at(idx).value.option === '' ||
      this.answerOptions.at(idx).value.optionWeightage === ''
    ) {
      ...
      if (idx === 0) {
        ...
        if (count > 1) {
          ...
          this.answerOptions
            .at(idx)
            .patchValue({ optionWeightage: '0', deleted: true });
        }
      } else {
        let count = 0;
        for (let i = 0; i < this.answerOptions.length; i++) {
  ...
  onSubmit() {
    ...
    if (this.editQuestionnaireForm.value.questionWeight === ' ') {
      ...
      for (
        let i = 0;
        i < this.editQuestionnaireForm.value.answerOptions.length;
        i++
      ) {
    ...
    const questionObj = {
      questionnaireDetail: {
        ...
        question:
          this.editQuestionnaireForm.value.questionName !== undefined &&
          this.editQuestionnaireForm.value.questionName !== null
            ? this.editQuestionnaireForm.value.questionName.trim()
            : null,
    ...
    this.questionnaire_service.editQuestionnaire(questionObj).subscribe(
      (resp) => {
        if (resp.valueOf()) {
  ...
  enableoptionField() {
    ...
    if (
      this.editQuestionnaireForm.value.answerType === 'Radio' ||
      this.editQuestionnaireForm.value.answerType === 'Dropdown'
    ) {
      const questList = <FormArray>(
        this.editQuestionnaireForm.controls['answerOptions']
      );
