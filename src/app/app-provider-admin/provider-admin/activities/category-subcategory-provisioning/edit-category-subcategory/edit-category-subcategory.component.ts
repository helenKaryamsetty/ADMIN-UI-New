...
export class EditCategorySubcategoryComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<EditCategorySubcategoryComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,
    private catService: CategorySubcategoryService,
  ) {}
  ngOnInit() {
  ...
  editCategory() {
    ...
    this.catService.editCategory(catObj).subscribe(
      (response) => {
        if (response) {
  ...
  editSubCategory() {
    ...
    catObj['modifiedBy'] = this.commonData.uname;
    this.catService.editSubCategory(catObj).subscribe(
      (response) => {
        if (response) {
          this.message.alert('Updated successfully', 'success');
          this.dialogRef.close(true);
    ...
    if (
      ...
      subCategoriesExist = this.subcategories.filter((obj: any) => {
        return (
          ...
          obj.subCategoryName.toString().toLowerCase().trim() ===
