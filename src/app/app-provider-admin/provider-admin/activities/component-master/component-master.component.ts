...
export class ComponentMasterComponent implements OnInit {
  ...
  userID: any;
  services_array: any = [];
  checkradioButton: boolean = false;
  checktextbox: boolean= false;
  state: any;
  service: any;
  ...
  constructor(private commonDataService: dataService,
    ...
    public stateandservices: ServicePointMasterService,public dialog: MatDialog) {
    this.states = [];
    this.services = [];
  ...
  */
  ...
  componentUnique() {
    ...
    for (let a = 0; a < this.filteredComponentList.data.length; a++) {
      if (this.filteredComponentList.data[a].testComponentName === this.testComponentName) {
        count = count + 1;
 ...
 this.updateComponentDet();
        ...
        for (let key in item) {
          if (key == 'testComponentName' || key == 'inputType') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredComponentList.data.push(item); break;
            }
          }
        }
      });
  ...
  getComponentForm(): AbstractControl[] | null {
    const componenListControl = this.componentForm.get(
      'compOpt'
    );
