...
export class DeviceIdMasterComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SNo', 'deviceID', 'DeviceName','edit','action'];
...
deactivateDeviceIdMaster(data:any, isDeleted:any) {
  ...
  addObj(deviceId:any, deviceName:any) {
    ...
    if (this.bufferArray.data.length === 0 && (obj.deviceID !== "" && obj.deviceID !== undefined)) {
  ...
  checkDeviceIDExistInBufferArray(obj:any)
    {
      ...
      let count = 0;
      for (var value of this.bufferArray.data) {
  ...
  saveDeviceMasterDetails() {
    this.fetosenseDeviceMasterService.saveFetosenseDeviceMaster(this.bufferArray.data)
      .subscribe((response:any) =>
       {
        if (response && response.statusCode === 200 && response.data) {
