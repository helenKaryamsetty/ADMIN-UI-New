...
export class DrugStrengthComponent implements OnInit {
  ...
  getAllDrugStrength() {
    this.drugStrengthService.getDrugStrength().subscribe((response:any) => {
      if (response) {
  ...
  checkDuplicates(object:any) {
    ...
    if (this.drugStrengthList.data.length === 0) {
      ...
      for (let i = 0; i < this.drugStrengthList.data.length; i++) {
        if (this.drugStrengthList.data[i].drugStrength === object.drugStrength
        ) {
  ...
  saveDrugStrength() {
    ...
    this.drugStrengthService.saveDrugStrength(this.drugStrengthList.data).subscribe((response:any) => this.successHandler(response), err => {
        ...
        for (let key in item) {
          if (key == 'drugStrength') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredDrugStrength.data.push(item); break;
            }
          }
        }
      });
