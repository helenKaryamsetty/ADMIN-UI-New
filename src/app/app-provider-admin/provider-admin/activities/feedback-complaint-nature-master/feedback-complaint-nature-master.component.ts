...
export class FeedbackComplaintNatureMasterComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SNo', 'FeedbackNature', 'FeedbackNaturekDescription','edit','action'];
  displayedColumns1: string[] = ['SNo', 'FeedbackNature', 'FeedbackNaturekDescription','edit'];
  ...
  saveComplaintNature() {
    ...
    for (var i = 0; i < this.objs.data.length; i++) {
      var tempObj:any= {
        'feedbackNature': (this.objs.data[i].feedbackNature !== undefined && this.objs.data[i].feedbackNature !== null) ? this.objs.data[i].feedbackNature.trim() : null,
        'feedbackNatureDesc': (this.objs.data[i].feedbackNatureDesc !== undefined && this.objs.data[i].feedbackNatureDesc !== null) ? this.objs.data[i].feedbackNatureDesc.trim(): null,
        'feedbackTypeID': this.feedbackTypeID,
        ...
        for (let key in item) {
          if (key == 'feedbackNature') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filterednatureTypes.data.push(item); break;
            }
          }
        }
      });
...
export class EditFeedbackNatureModal {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, public dialog: MatDialog,
    ...
    public dialog_Ref: MatDialogRef<EditFeedbackNatureModal>,
    private alertService: ConfirmationDialogsService) { }
