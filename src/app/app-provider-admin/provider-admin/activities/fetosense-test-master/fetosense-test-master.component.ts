...
export class FetosenseTestMasterComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SNo', 'TestName', 'TestDescription','edit','action'];
  ...
  ngOnInit(): void {
          ...
          for (let key in item) {
            if (key == 'testName' || key == 'testDesc') {
              ...
              if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
                this.filteredFetosenseTests.data.push(item); break;
              }
            }
          }
        });
    ...
    validateAddedTest(test:any, desc:any){
      if (this.addedFetosenseTests.data.length < 1) {
        ...
        for(let addedTest of this.addedFetosenseTests.data){
    ...
    saveTests(){     
      this.providerAdminTestMasterService.createTests(this.addedFetosenseTests.data)
      .subscribe((response:any) => this.testStatusSuccessHandeler(response, 'save'), err => {
        this.alertService.alert('error' ,err);
      });
    }
    ...
    removeTest(index:any) {
      ...
      if(this.addedFetosenseTests.data.length === 0)
        this.saveTest=false;
    }
      ...
      if (flag) {
       ...
       this.alertService.confirm('Confirm', 'Are you sure you want to ' + this.confirmMessage + '?').subscribe((res) => {
         if (res) {
          ...
          .subscribe((response:any) => this.testStatusSuccessHandeler(response, this.confirmMessage), err => {
            this.alertService.alert('error' ,err);
          });
         }
       },
    ...
    validateTest(test: any): boolean {
      if (this.selectedTest !== undefined && this.selectedTest !== null && (this.selectedTest.trim().toUpperCase() === test.trim().toUpperCase())) {
        ...
        for (this.filteredTest of this.searchedFetosenseTests) {
          if (this.filteredTest.testName !== undefined && this.filteredTest.testName !== null && test !== undefined && test !== null && (this.filteredTest.testName.trim().toUpperCase() === test.trim().toUpperCase())) {
            count = count + 1;
          }
        }
        console.log(count);
