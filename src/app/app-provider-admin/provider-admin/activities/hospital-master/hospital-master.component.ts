  ...
  @ViewChild(MatPaginator) paginator: MatPaginator | null = null;
   ...
   timerSubscription!: Subscription;
     ...
     let requestData = {
        ...
        this.HospitalMasterService.postFormData(requestData)
          ...
          (response:any) => {
           ...
           console.log("Response", response)
            if (response.statusCode == 5000 && response.errorMessage) {
                console.log("Hello");
                this.alertService.confirm(response.data, 'error')
                .subscribe(() => {
                  this.uploadForm.resetForm();
                })
            }
          ...
          (error) => {
            ...
            // loaderDialog.close();
            console.log(error);
            this.alertService.confirm('Error while uploading excel file', 'error')
              .subscribe(() => {
                this.uploadForm.resetForm();
                this.file = undefined;
              })
    ...
    getServices(stateID:any) {
        ...
        this.HospitalMasterService.getServices(this.userID)
            .subscribe((response:any) => this.getServiceSuccessHandeler(response),
                (err) => {
    ...
    getDistrict(stateID:any) {
        ...
        this.HospitalMasterService.getDistricts(stateID).subscribe((response:any) => this.getDistrictSuccessHandeler(response),
            (err) => {
                console.log("Error", err);
                //this.alertService.alert(err, 'error')
            });
    ...
    getTaluk(districtID:any) {
        ...
        this.HospitalMasterService.getTaluks(districtID).subscribe((response:any) => this.getTalukSuccessHandeler(response),
            (err) => {
                console.log("Error", err);
                // this.alertService.alert(err, 'error')
            });
   ...
   /* setProviderServiceMapID(providerServiceMapID) {
    ...
    setProviderServiceMapID(providerServiceMapID:any) {
        ...
        .subscribe((response:any) => this.instituteSuccessHandeler(response), err => {
          console.log("Error", err);
          // this.alertService.alert(err, 'error');
        })
    }
    ...
    getInstitutions() {
        ...
        this.HospitalMasterService.getInstitutions(request_obj).subscribe((response:any) => this.getInstitutionSuccessHandeler(response),
            (err) => {
                console.log("Error", err);
                //this.alertService.alert(err, 'error')
            });
        ...
        if (toBeDeactivatedFlag === true) {
            this.alertService.confirm('Confirm', "Are you sure you want to Deactivate?").subscribe((response:any) => {
                if (response.data) {
                    ...
                    this.HospitalMasterService.deleteInstitution(obj).subscribe((response:any) => this.deleteInstitutionSuccessHandeler(response, "Deactivated"),
                        (err) => {
                            console.log("Error", err);
                            // this.alertService.alert(err, 'error')
                        });
    ...
    createInstitution() {
        ...
        this.HospitalMasterService.saveInstitution(request_Array).subscribe((response:any) => this.saveInstitutionSuccessHandeler(response),
            (err) => {
                console.log("Error", err);
                //this.alertService.alert(err, 'error')
            });
...
export class EditHospitalModal {
    ...
    constructor(@Inject(MAT_DIALOG_DATA) public data: any, public dialog: MatDialog,
        ...
        public dialogReff: MatDialogRef<EditHospitalModal>) { }
    ...
    update() {
        ...
        this.HospitalMasterService.editInstitution(edit_request_obj).subscribe((response:any) => this.editInstitutionSuccessHandeler(response),
            (err) => {
                console.log("Error", err);
                // this.alertService.alert(err, 'error')
            });
