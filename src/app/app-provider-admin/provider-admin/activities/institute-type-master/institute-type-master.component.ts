...
export class InstituteTypeMasterComponent implements OnInit {
  ...
  getServices(userID:any) {
    this._instituteTypeMasterService.getServicesForInstTypeMaster(userID)
      .subscribe((response:any) => this.getServicesSuccessHandeler(response), err => {
  ...
  getStates(serviceID:any, isNational:any) {
    this._instituteTypeMasterService.getStates(this.userID, serviceID, isNational)
      .subscribe((response:any) => this.getStatesSuccessHandeler(response, isNational), err => {
        console.log("Error", err);
        // this.alertService.alert(err, 'error');
      });
  }
  ...
  search(providerServiceMapID:any) {
    this._instituteTypeMasterService.getInstitutesType(providerServiceMapID)
      .subscribe((response:any) => this.searchSuccessHandeler(response), err => {
        console.log("Error", err);
        // this.alertService.alert(err, 'error');
      })
  }
      ...
      this.alertService.confirm('Confirm', "Are you sure you want to Deactivate?").subscribe(response => {
        if (response) {
          ...
          this._instituteTypeMasterService.toggle_activate_InstituteType(obj)
            .subscribe((response:any) => this.toggleActivateSuccessHandeler(response, "Deactivated"), err => {
              console.log("Error", err);
              // this.alertService.alert(err, 'error');
            })
        }
    ...
    if (isDeleted === false) {
      this.alertService.confirm('Confirm', "Are you sure you want to Activate?").subscribe(response => {
        if (response) {
          ...
          this._instituteTypeMasterService.toggle_activate_InstituteType(obj)
            .subscribe((response:any)=> this.toggleActivateSuccessHandeler(response, "Activated"), err => {
              console.log("Error", err);
              // this.alertService.alert(err, 'error');
            })
        }
  ...
  add_obj(institute_type:any, description:any) {
    ...
    if (this.bufferArray.data.length == 0 && (obj.institutionType != "" && obj.institutionType != undefined)) {
  ...
  save() {
    this._instituteTypeMasterService.saveInstituteType(this.bufferArray.data)
...
export class EditInstituteType {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, public dialog: MatDialog,
    ...
    public commonDataService: dataService,
    public dialogReff: MatDialogRef<EditInstituteType>) { }
  ...
  update(edited_institute_type_name:any, edited_institute_type_description:any) {
    let obj = {
    ...
    this._instituteTypeMasterService.editInstituteType(obj)
      .subscribe((response:any) => this.updateSuccessHandeler(response), err => {
