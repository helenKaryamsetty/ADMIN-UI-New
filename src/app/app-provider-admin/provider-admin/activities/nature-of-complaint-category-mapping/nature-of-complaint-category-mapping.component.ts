...
export class NatureOfComplaintCategoryMappingComponent implements OnInit {
  ...
  searchForm!: NgForm;
  displayedColumns: string[] = ['SNo', 'NatureOfComplaint', 'Category','edit','action'];
  displayedColumns1: string[] = ['SNo', 'NatureOfComplaint', 'Category','edit'];
  ...
  getServicelines() {
    this.complaintMappingService.getServiceLines(this.userID).subscribe((response:any) => {
  ...
  getFeedbackTypes(providerServiceMapID:any) {
    ...
    this.complaintMappingService.getFeedbackTypes(providerServiceMapID).subscribe((response:any) => {
  ...
  getComplaintTypeCategoryMapping(providerServiceMapID:any, feedbackNatureID:any) {
    let reqObj = {
  ...
  checkExistance(providerServiceMapID:any) {
    this.complaintMappingService.filterMappedCategory(providerServiceMapID).subscribe((response:any) => {
      ...
      if (!this.editable) {
        if (this.complaintCategoryMappingList.data.length > 0) {
          this.complaintCategoryMappingList.data.forEach((complaintMappingList:any) => {
      ...
      // on edit - populate the non mapped categories
        ...
        if (this.editCategoryMappingValues != undefined) {
          let tempCategory = this.categories.filter((categoryResponse:any) => {
  ...
  saveCategoryMapping() {
    ...
    let tempObj = this.complaintCategoryMappingList.data;
    this.complaintMappingService.saveComplaintToCategoryMapping(tempObj).subscribe((response:any) => this.successHandler(response));
  }
  ...
  updateCategoryMapping(formValue:any) {
    ...
    this.complaintMappingService.updateComplaintCategoryMapping(updateReqObj).subscribe((response:any) => {
  ...
  unmappingCategory(categoryID:any) {
    this.alertService.confirm('Confirm', "Are you sure you want to Unmap ?").subscribe((response:any) => {
      if (response) {
        let unmapObj = {
          "categoryID": categoryID,
          "modifiedBy": this.createdBy
        }
        this.complaintMappingService.unmapCategory(unmapObj).subscribe((response:any) => {
        ...
        for (let key in item) {
          if (key == 'categoryName') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredMappings.data.push(item); break;
            }
          }
        }
      });
