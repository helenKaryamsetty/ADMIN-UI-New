...
export class ParkingPlaceComponent implements OnInit {
    ...
    displayedColumns = [
      ...
      displayAddedColumns = ['sno', 'stateName', 'zoneName', 'parkingPlaceName','parkingPlaceDesc','areaHQAddress', 'action'];
    ...
    getServiceLines() {
        this.parkingPlaceMasterService.getServiceLinesNew(this.userID).subscribe((response:any) => {
    ...
    getParkingPlaceSuccessHandler(response:any) {
        ...
        for (let availableParkingPlace of this.availableParkingPlaces) {
            this.availableParkingPlaceNames.data.push(availableParkingPlace.parkingPlaceName);
        }
    }
    ...
    checkDuplicates(parkingPlaceObj:any) {
        if (this.parkingPlaceList.data.length == 0) {
            ...
            for (let a = 0; a < this.parkingPlaceList.data.length; a++) {
                if (this.parkingPlaceList.data[a].parkingPlaceName === this.parkingPlaceObj.parkingPlaceName
                    && this.parkingPlaceList.data[a].stateID === this.parkingPlaceObj.stateID
                    && this.parkingPlaceList.data[a].zoneID === this.parkingPlaceObj.zoneID
                    && this.parkingPlaceList.data[a].areaHQAddress === this.parkingPlaceObj.areaHQAddress) {
    ...
    storeParkingPlaces() {
        let obj = { "parkingPlaces": this.parkingPlaceList.data };
                ...
                for (let key in item) {
                    if (key == 'parkingPlaceName') {
                        ...
                        if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
                            this.filteredavailableParkingPlaces.data.push(item); break;
                        }
                    }
                }
            });
