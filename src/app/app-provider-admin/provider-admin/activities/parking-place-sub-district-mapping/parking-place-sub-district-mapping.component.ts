...
export class ParkingPlaceSubDistrictMappingComponent implements OnInit {
  ...
  displayAddedColumns = ['sno', 'parkingPlaceName', 'districtName', 'districtBlockName','action'];
  ...
  getParkingPlaceSubDistrictMappings(providerServiceMapID:any, zoneID:any, parkingPlaceID:any) {
    let mappedReqObj = {
      "providerServiceMapID": providerServiceMapID,
      "zoneID": zoneID,
  ...
  getDistrictsSuccessHandeler(districtResponse:any) {
    ...
    if (this.editMappedValue != undefined) {
      let editDistrict = this.districts.filter((editDistrictValue:any) => {
  ...
  getTaluksSuccessHandler(talukResponse:any, districtID:any, providerServiceMapID:any) {
  ...
  checkExistance(districtID:any, providerServiceMapID:any) {
    ...
    this.parkingPlaceMasterService.filterMappedTaluks(unmappedObj).subscribe((response:any) => {
      ...
      // on edit - populate the non mapped categories
        ...
        if (this.editMappedValue != undefined) {
          let editTaluk = this.taluks.filter((editTalukValue:any) => {
  ...
  saveSubdistrictMapping() {
    this.parkingPlaceMasterService.saveParkingPlaceSubDistrictMapping(this.mappingList.data)
  ...
  filterComponentList(searchTerm?: string) {
    if (!searchTerm) {
      this.filteredMappedParkingPlaceDistricts = this.mappedParkingPlaceDistricts.data;
    } else {
      this.filteredMappedParkingPlaceDistricts.data = [];
        ...
        for (let key in item) {
          if (key == 'districtName' || key == 'districtBlockName') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredMappedParkingPlaceDistricts.data.push(item); break;
            }
          }
        }
      });
