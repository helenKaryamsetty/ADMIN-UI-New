...
export class ProcedureMasterComponent implements OnInit {
  ...
  */
  ...
  procedureUnique() {
    ...
    for (let a = 0; a < this.filteredprocedureList.data.length; a++) {
      ...
      if (this.filteredprocedureList.data[a].procedureName === this.name && !this.filteredprocedureList.data[a].deleted) {
        count = count + 1;
  ...
  procedureUnique_actvate(name:any) {
    ...
    for (let a = 0; a < this.filteredprocedureList.data.length; a++) {
      ...
      if (this.filteredprocedureList.data[a].procedureName === name && !this.filteredprocedureList.data[a].deleted) {
        count = count + 1;
        console.log("count", count);
  ...
  saveProcedure() {
    debugger;
    let apiObject: any = this.objectManipulate();
    let count = 0;
    console.log('here to check available', apiObject);
    for (let a = 0; a < this.filteredprocedureList.data.length; a++) {
      if (this.filteredprocedureList.data[a].procedureName === apiObject["procedureName"] && !this.filteredprocedureList.data[a].deleted) {
        count = count + 1;
  ...
  */
   ...
   *
   */
  toggleProcedure(procedureID:any, index:any, toggle:any, procedureName:any) {
        ...
        for (let key in item) {
          if (key == 'procedureName' || key == 'procedureType') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredprocedureList.data.push(item); break;
            }
          }
        }
      });
...
this.procedureMasterServiceService.getDiagnosticProcedure().subscribe((response:any) => {
