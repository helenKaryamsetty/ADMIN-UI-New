...
export class ServicePointComponent implements OnInit {
    ...
    displayedColumns = [
      ...
      displayAddedColumns = ['sno', 'parkingPlaceName', 'districtName', 'servicePointName','servicePointDesc','servicePointHQAddress', 'action'];
    ...
    getDistrictsSuccessHandeler(response:any) {
        ...
        if (this.editServicePointValue != undefined) {
            let editDistrict = this.districts.filter((districtResponse:any) => {
    ...
    getServicePointSuccessHandler(response:any) {
        ...
        for (let availableServicePoint of this.availableServicePoints) {
            this.availableServicePointNames.data.push(availableServicePoint.servicePointName);
        }
    }
    ...
    SetTaluks(response: any) {
        ...
        if (this.editServicePointValue != undefined) {
            let editTaluk = this.taluks.filter((talukResponse: any) => {
    ...
    checkDuplicates(servicePointObj: any) {
        ...
        if (this.servicePointList.data.length === 0) {
            ...
            for (let i = 0; i < this.servicePointList.data.length; i++) {
                if (this.servicePointList.data[i].servicePointName === servicePointObj.servicePointName) {
                    count = count + 1;
                }
            }
            if (count === 0) {
    ...
    //* save method */
    storeServicePoints() {
        let obj = { "servicePoints": this.servicePointList.data };
        console.log(obj);
                ...
                for (let key in item) {
                    if (key == 'districtName' || key == 'blockName' || key == 'servicePointName') {
                        ...
                        if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
                            this.filteredavailableServicePoints.data.push(item); break;
                        }
                    }
                }
            });
