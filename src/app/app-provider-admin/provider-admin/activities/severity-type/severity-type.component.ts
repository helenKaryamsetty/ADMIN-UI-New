...
export class SeverityTypeComponent implements OnInit {
  ...
  getProviderServices() {
    this.severityTypeService.getServices(this.userID).subscribe((response:any) => {
  ...
  getProviderStates(serviceID:any, isNational:any) {
    this.severityTypeService.getStates(this.userID, serviceID, isNational).subscribe((response:any) => {
  ...
  findSeverity(psmID:any) {
    ...
    this.search = true;
    this.severityTypeService.getSeverity(this.providerServiceMapID).subscribe((response:any) => this.getSeveritysuccesshandler(response),
      (err) => {
  ...
  add(values:any) {
    ...
    if (this.severityArray.data.length == 0 && (obj.severityTypeName != "" && obj.severityTypeName != undefined)) {
      this.severityArray.data.push(obj);
    }
    else {
      let count = 0;
      for (let i = 0; i < this.severityArray.data.length; i++) {
        if (obj.severityTypeName === this.severityArray.data[i].severityTypeName) {
          count = count + 1;
        }
      }
      if (count == 0) {
  ...
  finalSubmit() {
    this.severityTypeService.addSeverity(this.severityArray.data).subscribe(response => this.createdSuccessHandler(response),
      (err) => {
        console.log("Error", err);
        //this.dialogService.alert(err, 'error')
      });
...
export class EditSeverityModalComponent {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,
    public dialog: MatDialog, public severityTypeService: SeverityTypeService, public alertService: ConfirmationDialogsService,
