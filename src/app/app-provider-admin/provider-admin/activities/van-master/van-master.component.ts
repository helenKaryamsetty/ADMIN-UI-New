...
export class VanComponent implements OnInit {
    ...
    displayedColumns = [
      ...
      displayAddedColumns = ['sno','stateName', 'parkingPlaceName','vanName','vehicalNo','vanType', 'action'];
  ...
  setDataSourceAttributes() {
    ...
    getStates(serviceID:any) {
        ...
        this.servicePointMasterService.getStates(this.userID, serviceID, false).
            subscribe((response :any) => this.getStatesSuccessHandeler(response, false), (err:any) => {
            });
    }
    ...
    obj: any;
    getVanTypes() {
        this.vanMasterService.getVanTypes().subscribe((response:any) => this.getVanTypesSuccessHandler(response));
    }
    ...
    getVans(providerServiceMapID:any, parkingPlaceID:any) {
        ...
        this.vanMasterService.getVans(this.vanObj).subscribe((response:any) => this.getVanSuccessHandler(response));
    ...
    checkDuplicates(vanObj:any) {
        ...
        if (this.vanList.data.length === 0) {
            ...
            for (let i = 0; i < this.vanList.data.length; i++) {
                if (this.vanList.data[i].vanName === vanObj.vanName
                    && this.vanList.data[i].providerServiceMapID === vanObj.providerServiceMapID
    ...
    storeVans() {
        let obj = { "vanMaster": this.vanList.data };
        console.log(obj);
        this.vanMasterService.saveVan(obj).subscribe((response:any) => this.vanSuccessHandler(response));
    }
    ...
    updateVanStatus(van:any) {
        ...
        this.alertMessage.confirm('Confirm', "Are you sure you want to " + vanString + "?").subscribe((response:any) => {
            if (response) {
                ...
                this.vanMasterService.updateVanStatus(this.dataObj).subscribe((response:any) => this.updateStatusHandler(response));
    ...
    updateVanData(van:any) {
        ...
        this.dataObj.modifiedBy = this.createdBy;
        this.vanMasterService.updateVanData(this.dataObj).subscribe((response:any) => this.updateHandler(response));
                ...
                for (let key in item)
                    if (key == 'vanName' || key == 'vehicalNo' || key == 'vanType') {
                        ...
                        if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
                            this.filteredavailableVans.data.push(item); break;
                        }
                    }
            });
        }
    ...
    back() {
        this.alertMessage.confirm('Confirm', "Do you really want to cancel? Any unsaved data would be lost").subscribe((res:any) => {
            if (res) {
                this.showList();
            }
        })
