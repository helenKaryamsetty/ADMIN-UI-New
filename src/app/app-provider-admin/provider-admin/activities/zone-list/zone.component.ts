...
export class ZoneComponent implements OnInit {
  ...
  displayAddedColumns = ['sno', 'zoneName', 'zoneDesc', 'stateName', 'zoneHQAddress', 'action'];
  ...
  checkDuplicates(zoneObj:any) {
    if (this.zoneList.data.length == 0) {
      ...
      for (let a = 0; a < this.zoneList.data.length; a++) {
        if (this.zoneList.data[a].zoneName === zoneObj.zoneName
  ...
  storezone() {
    let obj = { "zones": this.zoneList.data };
  ...
  updateZoneData(zone:any) {
    ...
    this.dataObj.modifiedBy = this.createdBy;
    this.zoneMasterService.updateZoneData(this.dataObj).subscribe((response: any) => {
