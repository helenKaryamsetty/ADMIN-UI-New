...
export class SwymedUserMappingComponent implements OnInit {
  ...
  */
  getAllSwymedUserDetails() {
    this.swymedUserConfigService.getSwymedUserDetails(this.serviceProviderID).subscribe((userResponse:any) => {
  ...
  */
  getAllDesignations() {
    this.swymedUserConfigService.getAllDesignations().subscribe((desigRes:any) => {
      if (desigRes != undefined) {
        this.designations = desigRes.data;
      }
      if (this.dataToBeEdit != undefined) {
        let editDesig = this.designations.filter((editDesigValue:any) => {
  ...
  getUserNameBasedOnDesig(designationID:any) {
    this.swymedUserConfigService.getUserName(designationID, this.serviceProviderID)
      .subscribe((response:any) => {
        ...
        if (this.dataToBeEdit != undefined) {
          let editUser = this.userNamesList.filter((editUserValue:any) => {
  ...
  */
  getVideoConsultationDomain() {
    this.swymedUserConfigService.getVideoConsultationDomain(this.serviceProviderID).subscribe((domainResponse:any) => {
      if (domainResponse != undefined) {
        this.domainList = domainResponse.data;
      }
      if (this.dataToBeEdit != undefined) {
        let editDomain = this.domainList.filter((editDomainValue:any) => {
  ...
  */
  ...
  saveUserMapping(formValue:any) {
    ...
    this.swymedUserConfigService.saveSwymedUserDetails(saveReqObj).subscribe((saveResponse:any) => {
  ...
  */
  ...
  updateUsermapping(formValue:any) {
    ...
    this.swymedUserConfigService.updateUserDetails(updateObj).subscribe((updateResponse:any) => {
        ...
        for (let key in item) {
          if (key == 'userName' || key == 'videoConsultationDomain' || key == 'designationName') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredswymedUserDetails.data.push(item); break;
            }
          }
        }
      });
