...
export class UserRoleAgentIDMappingComponent implements OnInit {
  ...
  getStates(serviceID:any, isNational:any) {
    this._UserRoleAgentID_MappingService.getStates(this.userID, serviceID, isNational)
      .subscribe((response:any) => this.getStatesSuccessHandeler(response, isNational),
  ...
  getServices(userID:any) {
    this._UserRoleAgentID_MappingService.getServices(userID)
      .subscribe((response:any) => this.getServicesSuccessHandeler(response),
        (err) => console.log(err, 'error'));
  }
  ...
  getRoles(providerServiceMapID:any) {
    ...
    this._UserRoleAgentID_MappingService.getRoles(providerServiceMapID)
      .subscribe((response:any) => this.rolesSuccesshandeler(response),
        (err) => console.log(err, 'error'));
  }
  ...
  searchEmployee(state:any, service:any, role:any, empname:any, empid:any) {
    ...
    this._UserRoleAgentID_MappingService.getEmployees(request_obj).subscribe((response:any) => this.getEmployeesSuccessHandeler(response),
      (err) => console.log(err, 'error'));
  }
  ...
  getEmployeesSuccessHandeler(response:any) {
    ...
    if (response) {
      ...
      this.filteredsearchResultArray.data = response.data.filter(function (obj:any) {
        return obj.uSRMDeleted == false && obj.roleName != 'ProviderAdmin';
      });
...
export class AgentIDMappingModal {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, public dialog: MatDialog,
    ...
    public commonDataService: dataService, public alertService: ConfirmationDialogsService,
    public dialogReff: MatDialogRef<AgentIDMappingModal>) {
  ...
  ngOnInit() {
    ...
    if (this.providerServiceMapID != undefined) {
      this._UserRoleAgentID_MappingService.getAvailableCampaigns(this.providerServiceMapID)
        .subscribe((response:any) => this.getAvailableCampaignsSuccessHandeler(response),
          (err) => console.log(err, 'error'));
    }
  ...
  getAgentIDs(campaign_name:any) {
    this._UserRoleAgentID_MappingService.getAgentIDs(this.providerServiceMapID, campaign_name)
      .subscribe((response:any) => this.getAgentIDsSuccessHandeler(response),
        (err) => console.log(err, 'error'));
  }
  ...
  setAgentPassword_usrAgentMappingID(agentPassword:any, usrAgentMappingID:any) {
  ...
  mapAgentID(agentID:any) {
    ...
    this._UserRoleAgentID_MappingService.mapAgentID(req_array)
      .subscribe((response:any) => this.mapAgentIDSuccessHandeler(response),
        (err) => console.log(err, 'error'));
  }
