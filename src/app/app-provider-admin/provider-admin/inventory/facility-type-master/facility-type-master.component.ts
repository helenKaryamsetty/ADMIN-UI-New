...
throw new Error('Method not implemented.');
  ...
  getServices() {
    this.commonservice.getServiceLines(this.commonDataService.uid).subscribe((response:any) => {
      if (response) {
  ...
  getstates(service:any) {
    this.facility.getStates(this.commonDataService.uid, service.serviceID, false).subscribe((response:any) => {
      if (response) {
  ...
  getAllFacilities(providerServiceMapID:any) {
    ...
    this.facility.getfacilities(providerServiceMapID).subscribe((response:any) => {
      if (response) {
        ...
        for (let key in item) {
          if (key == 'facilityTypeCode' || key == 'facilityTypeName' || key == 'facilityTypeDesc') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredfacilityMasterList.data.push(item); break;
            }
          }
        }
      });
  ...
  activate(facilityTypeID:any) {
    this.dialogService.confirm('Confirm', "Are you sure you want to Activate?").subscribe((response:any) => {
      if (response) {
        const object = {
  ...
  deactivate(facilityTypeID:any) {
    this.dialogService.confirm('Confirm', "Are you sure you want to Deactivate?").subscribe((response:any)=> {
      if (response) {
        const object = {
          "facilityTypeID": facilityTypeID,
          "deleted": true
  ...
  checkDuplictes(object:any) {
    ...
    if (this.bufferArray.data.length === 0) {
      ...
      for (let i = 0; i < this.bufferArray.data.length; i++) {
        if (this.bufferArray.data[i].facilityTypeCode === object.facilityTypeCode
        ) {
  ...
  saveFacilityTypes() {
    this.facility.savefacilities(this.bufferArray.data).subscribe(response => {
      if (response) {
