...
create_physicalLocation: any;
  ...
  getstates(service:any) {
    this.storeService.getStates(this.uid, service.serviceID, false).subscribe((response:any) => {
      if (response) {
  ...
  getFacilityType() {
    //this.providerServiceMapID = providerServiceMapID;
    this.storeService.getAllActiveFacilities(this.providerServiceMapID).subscribe((response:any) => {
      if (response) {
  ...
  getStoreType() {
    //this.providerServiceMapID = providerServiceMapID;
    this.storeService.getStoreType(this.providerServiceMapID).subscribe((response:any) => {
      if (response) {
      ...
      this.storesList.forEach((item:any) => {
        for (let key in item) {
          ...
          if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
            this.filteredstoresList.data.push(item); break;
          }
        }
        }
      });
  ...
  activateDeactivate(facilityTypeID:any, flag:any) {
    ...
    this.dialogService.confirm('Confirm', "Are you sure you want to " + this.confirmMessage + "?").subscribe((response:any) => {
      if (response) {
        const object = {
  ...
  checkDuplictes(object:any) {
    ...
    if (this.bufferArray.data.length === 0) {
      ...
      for (let i = 0; i < this.bufferArray.data.length; i++) {
        if (this.bufferArray.data[i].facilityCode === object.facilityCode &&
  ...
  saveStores() {
    ...
    this.storeService.saveStore(this.bufferArray.data).subscribe((response:any) => {
      if (response) {
  ...
  editFacility(editFormValues:any) {
    ...
    if (editFormValues.storeType === "MAIN") {
      ...
      this.storeType_arrayEdit = this.storeType_array.filter(function (store:any) {
  ...
  updateFacilityType(editedFormValues:any) {
    ...
    this.storeService.updateStore(editObj).subscribe((response:any) => {
      if (response) {
