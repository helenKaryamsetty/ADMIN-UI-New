...
export class CreateStoreMappingComponent implements OnInit {
  ...
  getAllStore(providerServiceMapID:any) {
    this.storeSubs = this.storeMappingService.getAllStore(providerServiceMapID).
      subscribe((response:any)=> {
        ...
        this.storeList = response.data.filter((item:any) => !item.facilityDeleted);
  ...
  getParkingPlace(providerServiceMapID:any) {
    this.parkingSubs = this.storeMappingService.getAllParkingPlace(providerServiceMapID).
      subscribe((response:any) => {
        ...
        this.mainParkingPlaceList = response.data.filter((item:any) => !item.deleted);
        this.parkingPlaceList = response.data.filter((item:any) => !item.deleted);
      }, (err) => {
        this.notificationService.alert(err, 'error')
  ...
  getVan(facilityID:any) {
    this.vanSubs = this.storeMappingService.getAllVan(facilityID).
      subscribe((response:any) => {
        ...
        this.vanList = response.data.filter((item:any) => !item.facilityID && !item.deleted);
  ...
  subscribeToMainStoreChange() {
    let temp:any = this.storeMappingForm.controls['storeMapping'] as FormGroup;
    temp.controls['facilityName'].valueChanges
      .subscribe((value:any) => {
        ...
        if (value && value.data.parkingPlaceID) {
          let temp:any = this.mainParkingPlaceList.filter((item:any) => item.parkingPlaceID == value.parkingPlaceID);
          if (temp.length > 0)
            (<FormGroup>this.storeMappingForm.controls['storeMapping']).controls['parkingPlaceName'].setValue(temp[0]);

          this.subStoreList = this.storeList.filter((item:any) => item.mainFacilityID == value.facilityID && item.isMainFacility == false && !item.vanID);
  ...
  subscribeToStoreSelectionChange() {
    ...
    temp.controls['isMainFacility'].valueChanges
      .subscribe((value:any) => {
        if (value) {
          this.mainStoreList = this.storeList.filter((item:any) => !item.parkingPlaceID && item.isMainFacility);
          this.parkingPlaceList = this.mainParkingPlaceList.filter((item:any) => !item.facilityID);
        } else {
          this.mainStoreList = this.storeList.filter((item:any) => item.isMainFacility);
  ...
  checkValidity() {
    let temp:any = this.storeMappingForm.controls['storeMapping'].value;
  ...
  addToStoreMappingList() {
    let temp:any = JSON.parse(JSON.stringify(this.storeMappingForm.value.storeMapping));
    ...
    if (temp) {
      let arr:any = this.storeMappingList.data.filter((item:any) => (temp.facilityID != undefined && item.facilityID == temp.facilityID) ||
  ...
  submitStoreMapping() {
    let temp:any = this.storeMappingList.data.slice();
