...
export class UpdateStoreMappingComponent implements OnInit {
  ...
  patchStoreDetails(storeDetails:any) {
    if (storeDetails.parkingPlaceID) {
      let mainStore = this.storeList.filter((item: any) => item.facilityID == storeDetails.facilityID);
    ...
    if (storeDetails.vanID) {
      let mainStore = this.storeList.filter((item:any) => item.facilityID == storeDetails.mainFacilityID);
      let subStore = this.storeList.filter((item:any) => item.facilityID == storeDetails.facilityID);
  ...
  getParkingPlace(providerServiceMapID:any, storeDetails:any) {
    this.parkingSubs = this.storeMappingService.getAllParkingPlace(providerServiceMapID).
      subscribe((response:any) => {
        ...
        let parkingPlace = response.data.filter((item:any) => item.parkingPlaceID == storeDetails.parkingPlaceID);
        this.parkingPlaceList = response.data.filter((item:any) => !item.facilityID || item.parkingPlaceID == storeDetails.parkingPlaceID);
        this.storeMappingForm.patchValue({
  ...
  getVan(facilityID:any, storeDetail:any) {
    this.vanSubs = this.storeMappingService.getAllVan(facilityID).
      subscribe((response:any) => {
        ...
        let van = response.data.filter((item:any) => item.vanID == storeDetail.vanID);
        this.vanList = response.data.filter((item:any) => !item.facilityID || item.vanID == storeDetail.vanID);
        this.storeMappingForm.patchValue({
          vanID: van[0].vanID,
          vanName: van[0]
        })
