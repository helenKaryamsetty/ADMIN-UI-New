...
import { CallTypeSubtypeService } from "../core/services/ProviderAdminServices/calltype-subtype-master-service.service";
...
export class CallDispositionTypeMasterComponent implements OnInit {
  ...
  isInbound = false;
  isOutbound = false;
  displayedColumns: string[] = ['SNo', 'CallType', 'CallSubType','Inbound','Outbound','MaxRedial','BlockingRequired','FollowupRequired','edit','action'];
  displayedColumnsTable2: string[] = ['SNo', 'CallGroupType', 'callType','Block','Followup','Inbound','Outbound','action'];
  ...
  constructor(public callTypeSubtypeService: CallTypeSubtypeService, private alertService: ConfirmationDialogsService,
    public commonDataService: dataService, public dialog: MatDialog) {
    this.data = [];
  ...
  ngOnInit() {
    ...
    this.callTypeSubtypeService.getServiceLinesNew(this.userID).subscribe((response:any) =>
  ...
  pushCallSubType(callType:any, call_subtype:any, fitToBlock:any, fitForFollowup:any) {
  ...
  save() {
    this.callTypeSubtypeService.saveCallTypeSubtype(this.temporarySubtypeArray.data)
      .subscribe((response:any) => this.saveCallTypeSubTypeSuccessHandeler(response), err => {
        console.log("error", err);
        // this.alertService.alert(err, 'error');
      });
  }
  ...
  get_calltype_subtype_history() {
    ...
    this.callTypeSubtypeService.getCallTypeSubType(this.providerServiceMapID)
      .subscribe((response:any) => this.getCallTypeSubTypeSuccessHandeler(response), err => {
        console.log("error", err);
        // this.alertService.alert(err, 'error');
      });
  }
...
export class EditCallType {
  ...
  CTS(callType:any) {
    this.tempCorrespondingSubCallType = [];
    this.tempCorrespondingSubCallType = this.tableData.filter(function (obj:any) {
      return obj.callGroupType == callType;
    });
  ...
  modify(value:any) {
    ...
    if (this.isInbound === false && this.isOutbound === false) {
      ...
      this.callTypeSubtypeService.modificallType(object).subscribe((response:any) => this.modifySuccess(response));
    }
  }
