...
import { CategorySubcategoryService } from 'src/app/core/services/ProviderAdminServices/category-subcategory-master-service.service'; 
@Component({
...
export class EditCategorySubcategoryComponent implements OnInit {
  ...
  constructor(private commonData: dataService,
    private message: ConfirmationDialogsService, public dialogRef: MatDialogRef<EditCategorySubcategoryComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any, private catService: CategorySubcategoryService) { }
  ngOnInit() {
  ...
  checkSubCategory(subCategoryName: string) {
    ...
    if (subCategoryName !== undefined && subCategoryName !== null && subCategoryName.trim().length > 0) {
      ...
      subCategoriesExist = this.subcategories.filter((obj:any) => {
