...
import { LanguageMapping } from '../core/services/ProviderAdminServices/language-mapping.service';
...
export class LanguageMappingComponent implements OnInit {
  ...
  displayedColumnsTable2: string[] = ['SNo', 'UserName', 'LanguageName','ReadWeightage','WriteWeightage','SpeakWeightage','action'];
  displayedColumns: string[] = ['SNo', 'UserName', 'Language','Read','Write','Speak','edit','action'];
  ...
  getUserName(providerId: any) {
    this.languageMapping.getUserName(providerId)
      ...
      .subscribe((response:any) => {
        if (response) {
          console.log('All User names under this provider Success Handeler', response.data);
  ...
  getAllMappedLanguagesList() {
    this.languageMapping.getMappedLanguagesList(this.serviceProviderID)
      .subscribe((response: any) => {
        if (response && response.data) { // Check if response and response.data are truthy
  ...
  getAvailableLanguages(username: any) {
    ...
    const filteredLanguages = this.allLanguagesList.filter(function (allLang:any) {
  ...
  saveMapping() {
    ...
    for (let i = 0; i < this.bufferArray.data.length; i++) {
      const index: number = duplicateArray.indexOf(this.bufferArray.data[i].userID);
      if (index == -1) {
        ...
        for (let j = 0; j < this.bufferArray.data.length; j++) {
          if (this.bufferArray.data[i].userID === this.bufferArray.data[j].userID) {
        ...
        for (let key in item) {
          if (key == 'userName' || key == 'languageName') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredLanguageMappedList.data.push(item); break;
            }
          }
        }
      });
