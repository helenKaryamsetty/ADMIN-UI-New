...
export class StoreSelfConsumptionServiceService {
  ...
  constructor(
    ...
    this.getItemBatchForStoreID =
      this.admin_Base_Url + 'getItemBatchForStoreID/';
  }
  getStoreItemsCall(facID: any) {
    return this.http.post(this.getStoreItems + facID, {});
    // .map(this.handleSuccess)
    //     .catch(this.handleError);
  }

  getItemBatchForStoreIDCall(itemID: any, facID: any) {
    return this.http.post(this.getItemBatchForStoreID, {
      facilityID: facID,
      itemID: itemID,
    });
    // .map(this.handleSuccess)
