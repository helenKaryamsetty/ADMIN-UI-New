...
import { WorkLocationMapping } from "../core/services/ProviderAdminServices/work-location-mapping.service";
import { VillageMasterService } from "../core/services/adminServices/AdminVillage/village-master-service.service";
...
export class WorkLocationMappingComponent implements OnInit {
  ...
  displayedColumns: string[] = ['SNo', 'UserName', 'Serviceline','State','District','Block','Village','WorkLocation','Role','Inbound','Outbound','edit','action'];
  displayedColumnsTable2: string[] = ['SNo', 'UserName', 'Serviceline','State','District','Block','Village','WorkLocation','Role','Inbound','Outbound','ESanjeevani','delete'];
...
item: any;
  ...
  getStates(serviceID: any, isNational: any) {
    ...
    this.worklocationmapping.getStates(this.userID, serviceID, isNational).
      subscribe((response:any) => this.getStatesSuccessHandeler(response.data, isNational), err => {
  ...
  showAlertsForMappedRoles(serviceID:any, userID:any, providerServiceMapID:any) {
    let reqObj = {
      "userID": userID,
      "providerServiceMapID": providerServiceMapID
    }
    this.worklocationmapping.getAllMappedRolesForTm(reqObj).subscribe((response:any) => {
  ...
  checkExistance(serviceID:any, providerServiceMapID:any, userID:any) {
    ...
    let tempsupAndSpecScreenNames:any = [];
    ...
    let bufferTempsupAndSpecScreenNames: any[] = [];
    this.availableRoles.forEach((screenNames:any) => {
  ...
  back() {
    this.alertService.confirm('Confirm', "Do you really want to go back? Any unsaved data would be lost").subscribe((response:any) => {
      if (response) {
        this.showTable();
        this.getAllMappedWorkLocations();
  ...
  activate(userID:any,serviceID:any, uSRMappingID:any, userDeactivated:any, providerServiceMappingDeleted:any, stateID: any, workingDistrictID: any, blockID: any, roleID: any ) {
    ...
    if (userDeactivated) {
      ...
      if (serviceID == 4) {
        this.alertService.confirm('Confirm', "Are you sure you want to Activate?").subscribe((response:any) => {
          if (response) {
            const object = {
  ...
  deactivate(serviceID:any, uSRMappingID:any) {
    if (serviceID == 4) {
      this.alertService.confirm('Confirm', "Are you sure you want to Deactivate?").subscribe((response:any) => {
        if (response) {
  ...
  addWorkLocation(objectToBeAdded: any, role:any) {
    ...
    if (objectToBeAdded.serviceline.serviceName === "1097") {
      if ((this.isInbound === false || this.isInbound === null || this.isInbound == undefined) && (this.isOutbound === false || this.isOutbound === null || this.isOutbound === undefined) && objectToBeAdded.role.some((item:any) => item.roleName.toLowerCase() !== "supervisor")) {
        this.alertService.alert('Select checkbox Inbound/Outbound/Both');
      }
      else {
        if (objectToBeAdded.role.some((item:any) => item.roleName.toLowerCase() === "supervisor") && (this.isOutbound == true || this.isInbound == true)) {
  ...
  setWorkLocationObject(objectToBeAdded:any, obj:any, InboundValue:any, OnboundValue:any) {
    let villageIDArr:any = [];
    let villageNameArr:any = [];
  ...
  removeRole(rowIndex:any, roleIndex:any) {
    ...
    this.getAllRoles(this.bufferArray.data[rowIndex].serviceID, this.bufferArray.data[rowIndex].providerServiceMapID, this.bufferArray.data[rowIndex].userID);
        ...
        'previleges': [
          {
            'ID': [

              {
                ...
                'inbound': this.bufferArray.data[i].serviceName == "1097" ? this.bufferArray.data[i].Inbound : null,

                'outbound': this.bufferArray.data[i].serviceName == "1097" ? this.bufferArray.data[i].Outbound : null
            ...
            'villageID':
              ...
              this.bufferArray.data[i].villageID,

            'villageName':

              this.bufferArray.data[i].villageName,
  ...
  getAllDistricts_duringEdit2(state: any, stateID: any, serviceID: any, isNational_edit:any, districtID:any, psmID:any, userID:any) {
  ...
  getAllWorkLocations_duringEdit2(stateID: any, serviceID: any, isNational_edit:any, districtID:any, psmID:any, userID:any) {
      ...
      .subscribe((response:any) => {
        ...
        if (this.edit_Details != undefined) {
          ...
          if (this.RolesList) {
            ...
            let edit_role = this.RolesList.filter((mappedRole:any) => {
  ...
  getProviderServices_edit(userID:any) {
    ...
    this.worklocationmapping.getServices(this.userID)
      ...
      .subscribe((response:any) => this.getServicesSuccessHandeler(response.data), err => {
  ...
  getProviderStates_duringEdit(serviceID:any, isNational:any) {
    ...
    this.worklocationmapping.getStates(this.userID, serviceID, isNational).
      ...
      subscribe((response:any) => this.getStatesSuccessHandeler_duringEdit(serviceID, response.data, isNational, true), err => {
  ...
  getProviderStates_duringPatchEdit(serviceID:any, isNational:any) {
    ...
    this.worklocationmapping.getStates(this.userID, serviceID, isNational).
      ...
      subscribe((response:any) => this.getStatesSuccessHandeler_duringEdit(serviceID, response, isNational, false), err => {
  ...
  getStatesSuccessHandeler_duringEdit(serviceID:any, response:any, isNational:any, blockVillageCheckFlag:any) {
  ...
  getAllWorkLocations_duringEdit(stateID: any, serviceID: any, isNational_edit:any, districtID:any) {
  ...
  updateWorkLocation(workLocations: any) {
    ...
    if (workLocations.serviceID === 1) {
      let updateRoleName = this.RolesList.filter((response:any) => {
...
let result = false;
        ...
        for (let key in item) {
          if (key == 'userName' || key == 'serviceName' || key == 'stateName' || key == 'workingDistrictName' || key == 'blockName' || key == 'villageName' || key == 'locationName' || key == 'roleName') {
            ...
            if (value.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0) {
              this.filteredmappedWorkLocationsList.data.push(item); break;
            }
          }
        }
      });
  ...
  showInboundOutboundEdit(value:any, role:any) {
    let editRoleName = this.RolesList.filter((response:any) => {
  ...
  getBlockMaster(District:any) {
    this.villagemasterService.getTaluks(District.districtID)
      .subscribe((response:any) => this.getTalukSuccessHandeler(response),
        (err:any) => {
          console.log("Error", err);
        });
  ...
  getVillageMaster(Serviceblock:any) {
    ...
    this.villagemasterService.getVillage(requestObject)
      .subscribe((response:any) => this.getVillageSuccessHandeler(response),
        (err:any) => {
          console.log("Error", err);
        });
  ...
  getEditVillageMaster(ServiceEditblock:any) {
    ...
    this.villagemasterService.getVillage(requestObject)
      .subscribe((response:any) => this.getEditVillageSuccessHandeler(response),
        (err:any) => {
          console.log("Error", err);
        });
  ...
  getEditVillagePatchMaster(ServiceEditblock:any) {
    ...
    this.villagemasterService.getVillage(requestObject)
      .subscribe((response:any) => this.getEditPatchVillageSuccessHandeler(response),
        (err:any) => {
          console.log("Error", err);
        });
